...
public class IDsHelper {
	...
	public static void runConfiguration(Configuration config) {
		...
		tux_chestID = config.get(itemids, "Tux_chest ID", 1011).getInt();
		tux_legsID = config.get(itemids, "Tux_legs ID", 1012).getInt();
		tux_bootsID = config.get(itemids, "Tux_boots ID", 1013).getInt();
